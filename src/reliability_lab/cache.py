...
class ResponseCache:
    ...
    def similarity(a: str, b: str) -> float:
        """Improved similarity using exact match fast path and token overlap fallback.
        ...
        left = set(a_clean.split())
        right = set(b_clean.split())
        ...
        try:
            ...
            if best_score >= self.similarity_threshold:
                if _looks_like_false_hit(query, best_cached_query):
                    self.false_hit_log.append({"query": query, "cached": best_cached_query, "score": best_score})
                    return None, best_score
                return best_value, best_score
